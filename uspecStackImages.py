...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Reads the Ultraspec [dd-mm-yyyy/runxxx.dat] files produces previews and stacks of the images')
	...
	frameBinSize = int(config.FRAME_STACK_BIN)
	
	runInfo = ultraspecClasses.runInfo(arg.runname)
	found = runInfo.loadFromJSON(config.RUNINFO)
	if not found:
		print "Could not get info for this run from the ultra.json file."
	...
	debug.write("Creating the first stacked image for initial source extraction... using %d frames"%frameBinSize, level = 2)
	frameRange = frameBinSize - startFrame + 1
			...
			for index, s in enumerate(sources):
				...
				for f in filteredSources:
					if  f[0]==newSource[0] and \
						...
						f[2]==newSource[2]: 
						new = False
				if new: filteredSources.append(newSource)
		
			allWindows[windowIndex].setSources(filteredSources)	
