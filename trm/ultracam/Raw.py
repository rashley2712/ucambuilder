...
class Rwin(object):
    ...
    to help readability of code. Sets attributes
    llx, lly, nx, ly representing the lower-left
    pixel of a window and its binned dimensions.
    Used in the 'wins' attribute of Rhead
    """
...
class Rhead (object):
    ...
    def __init__(self, run, server=False):
        ...
        if server:
            if URL is None:
                raise UltracamError('Rhead.__init__: no url for server found.' + 
...
def utimer(tbytes, rhead, fnum):
	...
	#print "We are in DRIFT mode here"
        wl     = rhead.win[0]
        xbin   = rhead.xbin
        ybin   = rhead.ybin
        nxu    = xbin*wl.nx
