...
class CountsFrame(tk.LabelFrame):
    ...
    def __init__(self, master, share):
        ...
        # entries
        self.filter    = drvs.Radio(lframe, ('u', 'g', 'r', 'i', 'z'), 3, self.update)
        self.filter.set('g')
        self.mag       = drvs.RangedFloat(lframe, 18., 0., 30., self.update, True, width=5)
        self.seeing    = drvs.RangedFloat(lframe, 1.0, 0.2, 20., self.update, True, width=5)
        self.airmass   = drvs.RangedFloat(lframe, 1.5, 1.0, 5.0, self.update, True, width=5)
        self.moon      = drvs.Radio(lframe, ('d', 'g', 'b'),  self.update)
    ...
    def counts(self, expTime, cycleTime, ap_scale=1.6):
        ...
        etc given current magnitude, seeing etc. Assumes
        aperture
        ...
        if not ipars.check():
            raise drvs.DriverError('drivers.CountsFrame.counts: invalid parameters')
